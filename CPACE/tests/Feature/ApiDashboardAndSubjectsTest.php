...
/**
 * Covers the mobile API's Dashboard and Subjects endpoints
 * (Api\DashboardApiController, Api\SubjectsApiController) - both are
 * read-only but must scope every figure to the authenticated student only.
 * Hand-built schema, same rationale as the other Feature tests in this
 * suite (migration set doesn't run cleanly from scratch).
 */
class ApiDashboardAndSubjectsTest extends TestCase
{
    private const TABLES = [
        'performance_records', 'quiz_sessions', 'questions', 'topics', 'subjects',
    ...
    public function test_the_dashboard_figures_never_reflect_another_students_activity(): void
    {
        ...
        DB::table('quiz_sessions')->insert([
            ...
            'total_items' => 100, 'correct_answers' => 100, 'started_at' => now(), 'completed_at' => now(),
        ]);
        DB::table('quiz_sessions')->insert([
        ...
        $response = $this->getJson('/api/dashboard', $this->tokenHeaders($me->id));

        $response->assertOk()->assertJsonPath('questions_attempted', 10);
    }

    public function test_the_subjects_list_reports_this_students_own_mastery_per_subject(): void
    {
        $me = $this->student('me@example.com');
        $other = $this->student('other@example.com');
        ...
        DB::table('performance_records')->insert(['student_id' => $me->id, 'topic_id' => $topicId, 'total_attempts' => 10, 'correct_count' => 8]);
        DB::table('performance_records')->insert(['student_id' => $other->id, 'topic_id' => $topicId, 'total_attempts' => 10, 'correct_count' => 1]);

        $response = $this->getJson('/api/subjects', $this->tokenHeaders($me->id));

        $response->assertOk();
        $subject = collect($response->json('subjects'))->firstWhere('id', $subjectId);
        $this->assertSame(80, $subject['mastery']);
        $this->assertTrue($subject['is_passing']);
    }

    public function test_the_subjects_endpoint_requires_a_valid_token(): void
    {
