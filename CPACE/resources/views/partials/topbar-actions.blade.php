...
<a href="{{ route('messages.index') }}" aria-label="Messages" title="Messages" style="position:relative;width:38px;height:38px;border-radius:10px;background:#fff;color:var(--primary);border:1px solid #e5e7eb;display:flex;align-items:center;justify-content:center;text-decoration:none;flex-shrink:0;">
    <i class="fas fa-comment-dots"></i>
    @if($unreadMessages > 0)
        <span style="position:absolute;top:-5px;right:-5px;min-width:17px;height:17px;padding:0 4px;border-radius:9px;background:#dc2626;color:#fff;border:2px solid #fff;font-size:8px;font-weight:700;display:flex;align-items:center;justify-content:center;">{{ $unreadMessages > 9 ? '9+' : $unreadMessages }}</span>
    @endif
</a>
