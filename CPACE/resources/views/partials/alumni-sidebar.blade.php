...
<style>
    ...
    .sidebar.collapsed .sidebar-nav li a { margin: 1px 6px; padding: 10px 0; justify-content: center; gap: 0; }
    ...
    .sidebar .avatar-sm img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; border-radius: inherit; }
    .sidebar .avatar-default {
        display: flex; align-items: center; justify-content: center;
        width: 100%; height: 100%;
        font-size: inherit; font-weight: inherit; color: inherit; letter-spacing: inherit; line-height: 1;
    }
    ...
    .main { margin-left: 230px; transition: margin-left 0.28s cubic-bezier(.4,0,.2,1); }
    .sidebar.collapsed ~ .main { margin-left: 68px; }

    @media (max-width: 900px) {
        .sidebar { width: 68px; }
        .sidebar .logo-text, .sidebar .portal-badge, .sidebar .sidebar-nav li a span,
        .sidebar .user-details, .sidebar .chevron-icon, .sidebar .nav-label { display: none; }
    ...
    .topbar-dropdown {
        ...
        background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; min-width: 185px;
        box-shadow: 0 6px 20px rgba(0,0,0,.12); z-index: 2000; overflow: hidden;
    }
    .topbar-dropdown.open { display: block; }
    .topbar-dropdown a, .topbar-dropdown button {
        display: flex; align-items: center; gap: 10px;
        padding: 11px 16px; font-size: 13px; font-family: 'Poppins', sans-serif;
        text-decoration: none; color: #1a202c; background: none; border: none; width: 100%;
        text-align: left; cursor: pointer; transition: background .2s;
        border-bottom: 1px solid #f5f5f5;
    }
    .topbar-dropdown a:last-child, .topbar-dropdown form:last-child button { border-bottom: none; }
    .topbar-dropdown a:hover, .topbar-dropdown button:hover { background: #f7fafc; }
    .topbar-dropdown a i, .topbar-dropdown button i { color: var(--primary); width: 16px; text-align: center; }
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-logo" id="sidebarCollapseBtn" title="Toggle sidebar">
        ...
        <div class="logo-text">
            ...
            <div class="portal-badge">Alumni</div>
        </div>
    </div>
...
(function () {
    ...
    if (localStorage.getItem('alumniSidebar') === 'true') {
        sidebar.classList.add('collapsed');
    }

    function toggle() {
        sidebar.classList.toggle('collapsed');
        localStorage.setItem('alumniSidebar', sidebar.classList.contains('collapsed'));
    }
