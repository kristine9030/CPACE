...
<head>
    ...
    <title>{{ $code }} Quizzes - CPACE Faculty</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .back-link { display:inline-flex; align-items:center; gap:7px; font-size:12.5px; font-weight:600; color:#888; text-decoration:none; margin-bottom:14px; transition:color .15s; }
        .back-link:hover { color:var(--primary); }

        .topbar { display:flex; justify-content:space-between; align-items:center; margin-bottom:22px; gap:16px; flex-wrap:wrap; position:relative; z-index:100; }
        .topbar-left { display:flex; align-items:center; gap:14px; }
        .subj-icon { width:46px; height:46px; border-radius:13px; background:var(--sc-base, var(--primary)); color:#fff; display:flex; align-items:center; justify-content:center; font-size:19px; flex-shrink:0; box-shadow:0 4px 10px -2px rgba(0,0,0,.25); }
...
<main class="main">
    <a href="{{ route('faculty.quizzes') }}" class="back-link"><i class="fas fa-arrow-left"></i> All subjects</a>

    <div class="topbar" style="--sc-base:{{ $theme['base'] }};">
        <div class="topbar-left">
            <span class="subj-icon"><i class="fas {{ $icon }}"></i></span>
            <div>
                <div class="page-title">{{ $code }} Quizzes</div>
                <div class="page-sub">{{ $name }}</div>
            </div>
        </div>
        <div class="topbar-right">
            @include('partials.topbar-actions')
            <a href="{{ route('faculty.quizzes.create', $subject ? ['subject' => $subject->id] : []) }}" class="btn btn-primary"><i class="fas fa-plus"></i> New Quiz</a>
        </div>
    </div>

    <div class="tabs">
        @foreach(['' => 'All', 'draft' => 'Drafts', 'published' => 'Published', 'closed' => 'Closed'] as $key => $label)
            <a href="{{ route('faculty.quizzes.subject', ['subject' => $subjectKey] + ($key ? ['status' => $key] : [])) }}" class="tab {{ ($status ?? '') === $key ? 'active' : '' }}">
    ...
    @if($quizzes->isEmpty())
        <div class="empty">
            ...
            @if($status)
                No {{ $status }} quizzes in {{ $code }}.
            @else
                No quizzes here yet. Click <strong>New Quiz</strong> to build the first one for {{ $code }}.
            @endif
        </div>
    @else
        <div class="grid">
                ...
                <div class="quiz-card">
                    <div class="qc-top">
                        <div>
                            ...
                            <div class="qc-subject">updated {{ $quiz->updated_at?->diffForHumans() }}</div>
                        </div>
